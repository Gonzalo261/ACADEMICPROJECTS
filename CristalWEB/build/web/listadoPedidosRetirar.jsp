    ...
    <body>
        <center><br>
            <div class="container fluid-5"> 
                <div class="row">
                    <div class="col-4">
                    <h4 align="left">Lista de Retiros pendientes</h4>
                                </div> 
                        <div class="col-8"> 
                    ...
                    <div class="row">
                        ...
                        <table class="table table-hover table-active table-secondary">
                            <tr><th>Fecha Retiro</th><th>Lugar</th><th>Cliente</th><th>Nº Contrato</th><th> Fecha Evento</th><th>Estado</th><th>Comentarios</th><th></th></tr>
                            <c:choose>
                                        <c:when test="${not empty pedidoRetirar}">  
                            <c:forEach items="${ pedidoRetirar }" var="item">
                                        <tr>
                                            <td>${ item.DTOPresupuesto.fechaRetiro } </td>
                                            ...
                                            <td> <a style="text-align:center; color:green" onclick="window.location = 'Retiros?modo=alta&idContrato=${ item.idContrato }'" class="btn btn-group-sm">Retirado</a></td>
                                       </tr>
                                        </c:forEach>
                                           </c:when>
                                    <c:otherwise>
    ...
    <script>
        function filtrar(num)
        {
            if(num===1)
            {
                ...
                window.location = "Retiros?modo=pendientesAnualFiltro&fechaA="+ fechaA+"&fechaB="+ fechaB; 
            }
            else window.location = "Retiros?modo=pendientesAnual";   
        }
